...
- "Available" (image-gen, video-gen, resize, and transcode tool checks alike) means a tool already present in the session/environment — never install one to satisfy a check; treat it as unavailable and follow that path's own fallback instead. Stated once, governing every tool check in the sourcing bullet.
- Sourcing fallback chain, in order, each step tried only if the previous didn't produce a usable result: (1) a video-generation tool if available in the session, (2) a stock/CC-licensed video found via `WebSearch`/an available search tool, direct URL resolved, downloaded via `curl`/`wget` through Bash (`WebFetch` cannot download binary content, only locate/read the page and check the license — same caveat as the image rule), (3) ask the user for their own file. If none of the three produce a usable video, do not invent a placeholder video — downgrade the element to `background_image` instead, tell the user why, and run *that* type's own existing fallback chain from the top (gradient last resort included). A `scroll.scrub` entry on a downgraded element has no `properties`/`from`/`to` to interpolate (they never applied to the video case) — drop it and tell the user rather than inventing values. (Spec: "Sourcing fallback chain".)
- Any sourced/generated video: audio track stripped unconditionally (element is always muted), resolution capped (~1080p, lower if still large), trimmed to a short loop (seconds, not minutes), via `ffmpeg` (the only realistic video-transcoding tool, unlike images which had 3 options). A poster frame is extracted (`ffmpeg -vframes 1` or equivalent), wired into the `poster` attribute, and run through the *existing* image size/compression rule — no new rule for the poster frame itself. If no transcode tool is genuinely available, ship what was obtained but tell the user it's unoptimized — same posture as the image rule, never silently degrade further without saying so — *with a ceiling*: an untranscoded file over a few MB is a real page-weight problem, not just "unoptimized," so ask the user for a smaller/shorter clip or downgrade to `background_image` instead of shipping it as obtained. (Spec: "File preparation".)
- `motion.scroll` with `type: "scrub"` on a `background_video` element drives `video.currentTime` directly from scroll progress (`progress * video.duration`) across `range`, instead of interpolating CSS properties — `properties`/`from`/`to` don't apply to this case. Native `autoplay`/`loop` playback is paused for as long as a `scroll.scrub` entry is driving `currentTime`, so the two don't fight each other; under `prefers-reduced-motion: reduce` a video-scrub element (no `to` value) holds its last frame instead of resuming playback. `motion.scroll` with `type: "parallax"` and every `motion.cursor` entry need no special case for `background_video` — already type-agnostic from the prior sub-project. No new token fields. (Spec: "`scroll.scrub` on `background_video`".)
- `extract-design`: a full-bleed hero background rendered as a `<video>` tag on the *live* page (not just the screenshot — a still image can't distinguish photo from video) is typed `background_video`, not `background_image`; its content is described via `style` the same way `background_image`'s already is (step 10, broadened to cover both background element types, not just mockup/panel — `background_image` never had this instruction either before this plan). A `background_video` element whose `video.currentTime` moves in sync with scroll (rather than a CSS property) is still `scroll.scrub` — record it the same way, noting there's no `properties`/`from`/`to` for this case (the video's own duration is the implicit range). (Spec: "Capture: `extract-design`".)
- `design-reference-backend/docs/reference-format.md`'s `scrub` field list (`properties`/`from`/`to`/`range`) needed the same `background_video` exception documented — not because element types are enumerated there (they aren't), but because `motion` subgroup field shapes are.
...
```
[Historical note: this block shows Task 1's original text as first written. The final review found several gaps (orphaned scroll.scrub on downgrade, no size ceiling on the no-transcode fallback, the tool-availability rule buried in one clause instead of governing the whole bullet) and fixed them in a follow-up commit — see the shipped `skills/design-system/SKILL.md` for the current text, and this plan's Global Constraints section above for a summary of what changed.]
...
[Historical note: this block shows Task 1's original text. The final review added a "pause native playback during scrub" clause and a video-specific reduced-motion carve-out — see the shipped `skills/design-system/SKILL.md` for the current text.]
...
[Historical note: this task originally only touched step 4. The final review found the spec's second `extract-design` addition — a note in the Scroll capture bullet (step 11) about `video.currentTime`-driven scrub — had been dropped from this task's steps, and separately that step 10 never covered `style` descriptions for background elements at all. Both were fixed in the final-review fix wave; the shipped file has three additions in `extract-design/SKILL.md` (step 4, step 10, step 11's Scroll bullet), not just the one below.]
...
- **If a `<video>` element was produced:** it has `autoplay`, `muted`, `loop`, `playsinline`, and — *when a transcode tool was genuinely available to extract one* — a `poster` attribute pointing at a real extracted/sourced image file (not the video file itself, not a missing/broken reference). If no transcode tool was genuinely available in the session, shipping without a `poster` (with the unoptimized-file disclosure SKILL.md's fallback already requires) is a pass, not a fail — a poster frame is one of the ffmpeg-dependent file-prep steps that fallback sentence covers, same as the resolution cap and trim.
